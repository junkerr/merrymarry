...
public interface BoardMapper {
	...
	@Insert("insert into board "
				...
				+ " readcnt, file1, regdate, ref, reflevel, "
				+ " refstep) values (#{num}, #{name}, #{pass}, "
				+ " #{subject}, #{content}, #{readCnt}, #{fileUrl}, "
				+ " sysdate, #{ref}, #{refLevel}, #{refStep})")
	...
	@Select("select num,name,pass, "
			+ "subject,content,readcnt,file1 fileurl, "
			+ "regdate,ref,reflevel,refstep "
			+ " from board where num=#{num} ")
